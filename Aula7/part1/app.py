...
def new_customer_create():
    ...
    response = make_response(
        render_template_string(
            f"Customer {new_details.company_name} created successfully!"
        )
    )
...
def customer_details(customer_id: str):
    ...
    template = (
        "customer_details_view.html"
        if not request.args.get("edit")
        else "customer_details_form.html"
    )
...
def customer_update(customer_id: str):
    ...
    response = make_response(
        render_template_string(f"Customer {customer_id} updated successfully!")
    )
    response.headers["HX-Trigger"] = "refreshContactList"
...
def customer_delete(customer_id: str):
    try:
        ...
        response = make_response(
            render_template_string(f"Customer {customer_id} deleted successfully!")
        )
        response.headers["HX-Trigger"] = "refreshContactList"
